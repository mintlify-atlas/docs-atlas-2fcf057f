...
Add entries to the crontab on the Docker VM (edit with `crontab -e`):

```bash
# Run Docker cleanup every Sunday at 02:00
0 2 * * 0 /home/user/homelab/scripts/prune_docker.sh >> /var/log/prune_docker.log 2>&1

# Run backup every night at 03:00
0 3 * * * /home/user/homelab/scripts/backup.sh >> /var/log/backup.log 2>&1

# Update host inventory every Sunday at 04:00
0 4 * * 0 /home/user/homelab/scripts/update_hosts.sh >> /var/log/update_hosts.log 2>&1
```
